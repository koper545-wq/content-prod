...
export default function CreatorAgreementsPage() {
  const [agreements, setAgreements] = useState<Agreement[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/me/agreements")
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-xl font-bold text-gray-900">Moje umowy</h2>
      </div>
      ...
      {agreements.length === 0 ? (
        <EmptyState
          ...
          title="Brak umów"
          description="Umowy pojawią się tutaj po zaakceptowaniu Twojej aplikacji przez restaurację."
        />
      ) : (
        <div className="space-y-3">
          {agreements.map((a) => (
            <Link key={a.id} href={`/creator/umowa/${a.id}`}>
              <Card className="p-4 hover:border-orange-200 transition-colors cursor-pointer">
                ...
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span>{a.restaurantCompanyName}</span>
